...
export class RestaurantsController {
  ...
  @Get()
  @ApiOperation({ summary: 'Barcha restaranlar ' })
  async get() {
    const restaurants = await this.restaurantsService.getAll();
    return plainToInstance(RestaurantResponseDto, restaurants);
  }

  @Get(':restaurantId/workers')
  @ApiOperation({ summary: 'Restaran ishchilari' })
  @ApiOkResponse({ description: '✅ Restorandagi ishchilar ro‘yxati' })
  @ApiBadRequestResponse({ description: '❌ Noto‘g‘ri so‘rov' })
  async workers(@Param('restaurantId') restaurantId: string) {
    return await this.restaurantsService.workers(restaurantId);
  }
  ...
  @ApiOperation({
    ...
    description:
      'Yangi restoran qo‘shish uchun ishlatiladi. Rasm fayl sifatida yuboriladi.',
  })
  @ApiConsumes('multipart/form-data')
  @ApiBody({ type: RestaurantCreateDto })
  @ApiResponse({
    status: 201,
    description: 'Restoran muvaffaqiyatli qo‘shildi',
  })
  ...
  async create(
    ...
    try {
      ...
      throw new BadRequestException(
        error.message || 'Restoran qo‘shishda xatolik yuz berdi',
      );
    }
  }

  @Get(':restaurantId')
  @ApiOperation({ summary: 'Bitta restaranni olish' })
  ...
  @Delete(':restaurantId')
  @ApiOperation({ summary: 'Restaranni o`chirish ' })
